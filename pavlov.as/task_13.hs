-- Задание:
-- В модуле Data.Functions определена полезная функция высшего порядка
--
-- on :: (b -> b -> c) -> (a -> b) -> a -> a -> c
-- on op f x y = f x `op` y
--
-- Она принимает четыре аргумента: бинарный оператор с однотипными аргументами (типа b), функцию f :: a -> b,
-- возвращающую значение типа b. и два значения типа a. Функция on применяет f дважды к двум значениям типа a и передает
